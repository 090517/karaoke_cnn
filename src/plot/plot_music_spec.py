...
relevant_files = [path.join(music_path, x) for x in os.listdir(music_path) if (
    os.path.isfile(path.join(music_path,x)) and x.endswith(".npy"))]
...
librosa.output.write_wav("../diagrams/rand_music_spec.wav",
                         audio,
                         sr = gs.sample_rate)
...
fig.savefig("../diagrams/rand_music_spec.png", format="png")
plt.show()
